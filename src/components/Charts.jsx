...
function Charts({ productsByCategory, allProducts, categories }) {
  // Mapeo slug -> nombre para etiquetas
  const categoryNames = {};
  categories.forEach((cat) => {
    categoryNames[cat.slug] = cat.name;
  });

  const barChartData = Object.keys(productsByCategory).map((categorySlug) => ({
    name: categoryNames[categorySlug] || categorySlug,
    productos: productsByCategory[categorySlug],
  }));
